...
function DefineMonthView(theTextObject) { // the month view construct function
	...
	this.GetOnlyName = function() { // create one and only name on the document
		...
		aryName[2] = "_goPreviousMonth"; // the month view go previous month
											// button's name postfix

		aryName[3] = "_goNextMonth"; // the month view go next month button's
										// name postfix
		...
		aryName[6] = "_DayList"; // keep the month view current day's element
									// name postfix
	...
	// <yy> or <yyyy> is year, <m> or <mm> is digital format month, <MMM> or
	// <MMMMMM> is character format month, <d> or <dd> is day, other char
	// unchanged
	...
	this.UnselectBgColor = "#FFFFFF"; // the month view default background
										// color
	...
	this.DayBdColor = this.UnselectBgColor; // the day unit border color,default
											// is this.UnselectBgColor
	...
	this.WeekendBgColor = this.UnselectBgColor; // the weekend background color,
												// default is
												// this.UnselectBgColor

	this.WeekendColor = this.ValidColor; // the weekend front color, default
											// is this.ValidColor

	this.YearListStyle = "font-size:12px; font-family:宋体;"; // the year list's
															// style

	this.MonthListStyle = "font-size:12px; font-family:宋体;"; // the month
																// list's style

	this.MonthName = new Array(); // month name list, font is include
									// this.MonthListStyle
	...
	this.TitleStyle = "cursor:default; color:#000000; background-color:"
			+ this.UnselectBgColor
			+ "; font-size:16px; font-weight:bolder; font-family:Times new roman; text-align:center; vertical-align:bottom;"; // the
																																// month
																																// view
																																// title
																																// area's
	...
	this.WeekName = new Array(); // week name list, font is include
									// this.TitleStyle
	...
	this.FooterStyle = "cursor:hand; color:#000000; background-color:"
			+ this.UnselectBgColor
			+ "; font-size:12px; font-family:Verdana; text-align:left; vertical-align:middle;"; // the
																								// month
																								// footer
																								// area's
																								// style

	this.TodayTitle = "今天:"; // today tip string, font is include
								// this.FooterStyle

	this.MonthBtStyle = "font-family:Marlett; font-size:12px;"; // the change
																// month button
																// style
	...
	this.MonthGridStyle = "border-width:1px; border-style:solid; border-color:#000000;"; // the
																							// month
																							// view
																							// main
																							// body's
	...
	this.HeaderStyle = "height:32px; background-color:menu;"; // the month
																// view header
																// area's style

	this.LineBgStyle = "height:10px; background-color:" + this.UnselectBgColor
			+ "; text-align:center; vertical-align:middle;"; // the month
																// view title
																// area and day
																// area compart
																// area
	...
	this.LineStyle = "width:90%; height:1px; background-color:#000000;"; // the
																			// month
																			// view
																			// title
																			// area
	...
	this.DayStyle = "cursor:hand; font-size:12px; font-family:Verdana; text-align:center; vertical-align:middle;"; // the
																													// month
																													// view
																													// day
																													// area's
	...
	this.OverDayStyle = "this.style.textDecoration='underline';"; // the mouse
																	// over a
																	// day style

	this.OutDayStyle = "this.style.textDecoration='none';"; // the mouse out a
															// day style

	this.GetoffsetLeft = function(theObject) { // return theObject's absolute
												// offsetLeft
	...
	this.GetoffsetTop = function(theObject) { // return theObj's absolute
												// offsetTop
	...
	this.GetMonthDays = function(theYear, theMonth) { // get theYear and
														// theMonth days number
	...
	this.SetDateFormat = function(theYear, theMonth, theDay) {// format a date
																// to
																// this.DateFormat
		...
		theDate = theDate.replace(/<MMM>/g, this.MonthName[tmpMonth].substr(0,
				3));
	...
	this.GetTextDate = function(theString) { // convert a string to a date,
												// if the string is not a date,
		...
		for (i = 1; i < end_at; i++) {
			if (tmpDate.charAt(i) == "0") {
				...
				if (tmpChar < "0" || tmpChar > "9") {
					tmpDate = tmpDate.substr(0, i - 1) + "-"
							+ tmpDate.substr(i + 1);
				}
			}
		}
		for (i = 0; i < 9; i++) {
			tmpDate = tmpDate.replace(this.MonthName[i].toLowerCase().substr(0,
					3), "-00" + (i + 1).toString() + "-");
		}
		for (i = 9; i < 12; i++) {
			tmpDate = tmpDate.replace(this.MonthName[i].toLowerCase().substr(0,
		...
		if (find_tag != "000") {
			...
			if (one_at <= 12) {
				if (two_at > this.GetMonthDays(tmpYear, one_at - 1)) {
					return (new Date(tmpYear, one_at - 1, this.GetMonthDays(
							tmpYear, one_at - 1)));
				}
				if (two_at > 12) {
			...
			if (two_at <= 12) {
				if (one_at > this.GetMonthDays(tmpYear, two_at - 1)) {
					return (new Date(tmpYear, two_at - 1, this.GetMonthDays(
							tmpYear, two_at - 1)));
				}
				if (one_at > 12) {
	...
	this.CreateYearList = function(MinYear, MaxYear) { // create year list
		...
		var theYear = 0;
		var theYearHTML = "<select id=\""
				+ theName
				+ "_YearList\" style=\""
				+ this.YearListStyle
				+ "\" tabIndex=\"-1\" onChange=\"document.jsMonthView.UpdateMonthGrid(this)\" onBlur=\"document.jsMonthView.DeleteMonthGrid()\">";
		for (theYear = MinYear; theYear <= MaxYear; theYear++) {
			theYearHTML += "<option value=\"" + theYear.toString() + "\">"
					+ theYear.toString() + "</option>";
		}
		theYearHTML += "</select>";
	...
	this.CreateMonthList = function() { // create month list
		...
		var theMonth = 0;
		var theMonthHTML = "<select id=\""
				+ theName
				+ "_MonthList\" style=\""
				+ this.MonthListStyle
				+ "\" tabIndex=\"-1\" onChange=\"document.jsMonthView.UpdateMonthGrid(this)\" onBlur=\"document.jsMonthView.DeleteMonthGrid()\">";
		for (theMonth = 0; theMonth < 12; theMonth++) {
			theMonthHTML += "<option value=\"" + theMonth.toString() + "\">"
					+ this.MonthName[theMonth] + "</option>";
		}
		theMonthHTML += "</select>";
		theMonthObject.outerHTML = theMonthHTML;
	}
	this.setDayList = function(theYear, theMonth, theDay) { // set the month
															// view show a date
		var theName = this.Name;
		...
		for (DayId = 0; DayId < 7; DayId++) {
			theDayHTML += " <td width=\"10%\">" + this.WeekName[DayId]
					+ "</td>";
		}
		theDayHTML += " </tr>";
		theDayHTML += " <tr>";
		theDayHTML += " <td colspan=\"7\" style=\"" + this.LineBgStyle + "\">";
		theDayHTML += " <table style=\"" + this.LineStyle
		...
		for (WeekId = 0; WeekId < 6; WeekId++) {
			...
			for (DayId = 0; DayId < 7; DayId++) {
				...
				if ((theYear == theCurrentDate.getFullYear())
						&& (theMonth == theCurrentDate.getMonth())
						&& (theCurrentDay == theCurrentDate.getDate())
						&& (offsetMonth == 0)) {
					theBdColor = this.TodayBdColor;
				}
				theStyle = "border:" + this.DayBdWidth + "px solid "
						+ theBdColor + "; color:" + theColor
						+ "; background-color:" + theBgColor + ";";
				theDayHTML += " <td style=\""
						+ theStyle
						+ "\" onMouseOver=\""
						+ this.OverDayStyle
						+ "\" onMouseOut=\""
						+ this.OutDayStyle
						+ "\" onMouseDown=\"document.jsMonthView.CreateMonthGrid("
						+ theYear.toString() + ", "
						+ (theMonth + offsetMonth).toString() + ", "
						+ theCurrentDay.toString() + ")\">";
		...
		theDayHTML += " <tr style=\"" + this.FooterStyle
				+ "\" onMouseDown=\"document.jsMonthView.CreateMonthGrid("
				+ theCurrentDate.getFullYear().toString() + ", "
				+ theCurrentDate.getMonth().toString() + ", "
				+ theCurrentDate.getDate().toString() + ");\">";
		theStyle = "border:" + this.DayBdWidth + "px solid "
				+ this.TodayBdColor + ";";
		theDayHTML += " <td style=\"" + theStyle + "\"><br></td>";
		theDayHTML += " <td colspan=\"6\"> "
				+ this.TodayTitle
				+ " "
				+ this.SetDateFormat(theCurrentDate.getFullYear(),
						theCurrentDate.getMonth(), theCurrentDate.getDate())
				+ "</td>";
		theDayHTML += " </tr>";
		theDayHTML += " </table>";
	...
	this.CreateMonthGrid = function(theYear, theMonth, theDay) { // refresh
																	// the month
																	// view to
																	// the date,
																	// main
	...
	this.UpdateMonthGrid = function(theObject) { // run
													// this.CreateMonthGrid() by
													// theObject
		var theTextObject = this.Source;
		if (theTextObject == null) {
			return;
		...
		case "_goPreviousMonth": // go previous month button
			...
			this.CreateMonthGrid(parseInt(theYearObject.value, 10), parseInt(
					theMonthObject.value, 10) - 1, parseInt(theDayObject.value,
					10));
		...
		case "_YearList": // year list
			this
					.CreateMonthGrid(parseInt(theYearObject.value, 10),
							parseInt(theMonthObject.value, 10), parseInt(
									theDayObject.value, 10));
			break;
	...
	this.DeleteMonthGrid = function() { // check document focus, if blur
										// this.Source then delete this
		var theName = this.Name;
		...
		if (tmpObject == null) { // delete the month view
			...
			if (isNaN(theDate)) {
				...
				this.Source.value = this.SetDateFormat(theDate.getFullYear(),
	...
	this.InitialMonthView = function() {
		...
		var theDivHTML = "<div id=\""
				+ theName
				+ "_MonthView\" onBlur=\"document.jsMonthView.DeleteMonthGrid();\">";
		theDivHTML += " <table width=\"" + this.Width.toString()
				+ "\" height=\"" + this.Height.toString() + "\" style=\""
				+ this.MonthGridStyle
		...
		theDivHTML += " <tr align=\"center\" style=\"" + this.HeaderStyle
				+ "\">";
		theDivHTML += " <td>";
		theDivHTML += " <input type=\"button\" tabIndex=\"-1\" style=\""
				+ this.MonthBtStyle
				+ "\" id=\""
				+ theName
				+ "_goPreviousMonth\" value=\""
				+ this.PreviousMonthText
				+ "\" onClick=\"document.jsMonthView.UpdateMonthGrid(this)\" onBlur=\"document.jsMonthView.DeleteMonthGrid()\">";
		theDivHTML += " </td>";
		theDivHTML += " <td>";
		...
		theDivHTML += " </select>";
		theDivHTML += " <input type=\"hidden\" id=\"" + theName
				+ "_DayList\" value=\"1\">";
		theDivHTML += " </td>";
		theDivHTML += " <td>";
		theDivHTML += " <input type=\"button\" tabIndex=\"-1\" style=\""
		...
		theDivObject.style.posTop = this.GetoffsetTop(this.Source)
				+ this.Source.offsetHeight;
		this.CreateYearList(this.MinYear, this.MaxYear);
		this.CreateMonthList();
		this.CreateMonthGrid(theCurrentDate.getFullYear(), theCurrentDate
				.getMonth(), theCurrentDate.getDate());
	}
}
function CreateMonthView(theTextObject) { // the month view create interface,
											// fire at element's onFocus event
...
function DeleteMonthView(theTextObject) { // the month view delete interface,
											// fire at element's onBlur event
	if (document.jsMonthView == null) {
		return;
	}
	document.jsMonthView.DeleteMonthGrid();
