...
<%DAO DAO=new DAO();
	...
	try{
		...
		if(CurrentCycle<exp.getDcParameters().getTotalCycle()){
			response.sendRedirect(basePath+"servlet/transform");
			return;
		}
	}catch(Exception e){
		out.println("读取session数据失败，");
	...
	try{
		...
		title+="该实验已经结束。";
	//	title+="该实验已经结束，实验总共有"+exp.getDcParameters().getTotalCycle()+"期。";
	}catch(Exception e){
...
<html>
	...
	<body>
		<form action="" name="form1" method="post"  data-genuitec-lp-enabled="false" data-genuitec-file-id="wc1-22" data-genuitec-path="/behaviourDC/WebRoot/studentpage/finished.jsp"/>
		...
		<table width="1000" border="0" align="center" cellpadding="0" cellspacing="0" class="table19">
			<tr>
				<td valign="top">
					<span style="font:bold;color:green;">
						查看零售商数据：
						<select style="width: 10em;" name="retailerId" id="retailerId" onchange="submit()">
							<%
							for(int i=1;i<=exp.getDcParameters().getNumberofRetailer();i++){
								if(Integer.parseInt(retailerId)==i){
									out.println("<option value=\""+i+"\" selected=\"selected\" >零售商"+i+"</option>");
								}else{
									out.println("<option value=\""+i+"\" >零售商"+i+"</option>");
								}
							}
							 %>
						</select>
					...
					<table cellpadding="0" cellspacing="0" class="dataBorderedTable" style="width:1000px;">
										<tr>
											...
											<th style="background-color: khaki">
												<a href="javascript:orderby('netIncome');"><font color="black">总得分</font></a>
											</th>											
											<th style="background-color: red">
												<a href="javascript:orderby('total');"><font color="black">累计总报酬</font></a>
											</th>
										...
										<tr class="noBgColor" onMouseOver="mouseOverColor(this);" onMouseOut="mouseOutColor(this);">
											...
											<td><strong><font color="#0000ff"><%=Decimal.getDecimal(Math.max(0,d.getGoodsIn()-d.getOrderIn()),1)%></font><br></strong></td>
											<td><%=Decimal.getDecimal(d.getNetIncome(),1)%></td>
											<td><%=Decimal.getDecimal(d.getTotal(),1)%></td>
										</tr>
										<%
											}
											}
